...
class MaintainSeries extends Model
{
    use HasFactory;
    protected $fillable = ['wm_id','doc_type','product_id','seri', 'is_sold' ,'in_id' ,'out_id' ];
    
    public static function check_seri_in_avaible($seri, $product_id)
    {
        $seri = trim ($seri);
        $query = "select * from maintain_series where product_id =". $product_id
