...
class Product {
    constructor(id,name, price,price_in,price_out,type, quantity,stock_quantity,url,seri,series,pricelist) {
        this.id = id;
      this.name = name;
      this.url = url;
      this.price = price;
    ...
    generateHTML()
    {
        ...
        + this.name+'</a> <br/><span class="form-help px-2"> Tồn kho: '+ tonkho+'</span> <br/><span style="color:red">'+this.loi +' <span></div> </div> </td> <td class="text-right"><div class="flex"><input type="text" style="width:150px" id="ip'
        +this.id+'" onchange="updatePrice('+this.id+')" class="ipprice py-3 px-4  text-right form-control  " oninput="formatNumberS(this)" value="'
        + Intl.NumberFormat().format(this.price)+'"/> '+ dropdown+' </div></td><td class="text-right"><input type="number" style="width:100px"  id="iq'
  ...
  function updatePrice(id )
{
    ...
    productList.forEach((product) => {
        if(product.id == id)
        {
            ...
            if(price > product.price_out)
                product.loi = "";
            else
                if(price < product.price_in)
                    product.loi = 'giá thấp hơn giá nhập '+ Intl.NumberFormat().format(product.price_in);
                else
                    product.loi = 'giá thấp hơn giá bán trung bình '+ Intl.NumberFormat().format(product.price_out);
