...
<div class="content">
    @include('backend.layouts.notification')
                ...
                <div   class=" intro-y col-span-12 flex flex-col sm:flex-row sm:items-end xl:items-start">
                    <form action="{{route('inventory.print')}}" method = "get" class="xl:flex sm:mr-auto" >
                        <!-- @csrf -->
                        <div class="sm:flex items-center sm:mr-4">
                            <label style="min-width:80px" class="w-12 flex-none xl:w-auto xl:flex-initial mr-5">Lọc: </label>
                            <select name="wh_id" id="tabulator-html-filter-field" class="form-select w-full sm:w-32 2xl:w-full mt-2 sm:mt-0 sm:w-auto">
                                <option value="0">-chọn kho-</option>
                                @foreach ($whs as $wh )
                                    <option value="{{$wh->id}}" {{$wh_id==$wh->id?'selected':''}}>{{$wh->title}}</option>
                                @endforeach
                            </select>
                ...
                <div id="divprint" class="intro-y box overflow-hidden mt-5">
                    <div class="border-b border-slate-200/60 dark:border-darkmode-400 text-center sm:text-left">
                        <div class="px-5 py-10 sm:px-20 sm:py-10">
                            <div class="text-primary font-semibold text-3xl">DANH SÁCH TỒN KHO</div>
                        ...
                        <div class="flex flex-col lg:flex-row px-5 sm:px-20 pt-10 pb-5 sm:py-5">
                            <table class="table" style="width: 100%">
                                <thead class="table-dark"> 
                                    ...
                                    <td class="whitespace-nowrap"> Số lượng </td>
                                    <td class="whitespace-nowrap"> Kho </td>
                                </thead>
                                @foreach ($inventorys as $inv)
                                    <tr>
                                        <td> {{$i}} </td>
                                        <td style="width: 50%"> {{$inv->title}} </td>
                                        <td> {{$inv->quantity}} </td>
                                        <td> {{$inv->whtitle}} </td>
                                    </tr>
                                    <?php $tong += $inv->quantity; $i += 1;?>
                                @endforeach
                                <thead class="table-dark"> 
                                <tr> <td colspan='2'> Tổng </td> <td> {{number_format($tong,0,",",".")}} </td> <td> </td></tr>
                                <thead>
                            </table>
