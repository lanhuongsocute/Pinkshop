                        ...
                        <div class="mt-3">
                                ...
                                <select id="wh_id1" name="wh_id1" class="form-select mt-2 sm:mr-2"    >
                                    @foreach ($warehouses as $wh)
                                        <option value="{{$wh->id}}" {{$warehousetrans->wh_id1==$wh->id?'selected':''}}>{{$wh->title}}</option>
                            ...
                            <div class="mt-3">
                                ...
                                <select id="vendor_id1" name="vendor_id1" class="form-select mt-2 sm:mr-2"    >
                                    @foreach ($vendors as $vd)
                                        <option value="{{$vd->id}}" {{$warehousetrans->vendor_id1==$vd->id?'selected':''}}>{{$vd->full_name}}</option>
                            ...
                            <div class="mt-3">
                                ...
                                <select id="wh_id2" name="wh_id2" class="form-select mt-2 sm:mr-2"    >
                                    @foreach ($warehouses as $wh)
                                        <option value="{{$wh->id}}" {{$warehousetrans->wh_id2==$wh->id?'selected':''}}>{{$wh->title}}</option>
                            ...
                            <div class="mt-3">
                                ...
                                <select id="vendor_id2" name="vendor_id2" class="form-select mt-2 sm:mr-2"    >
                                    @foreach ($vendors as $vd)
                                        <option value="{{$vd->id}}" {{$warehousetrans->vendor_ids==$vd->id?'selected':''}}>{{$vd->full_name}}</option>
                            ...
                            <div class="mt-3">
                                ...
                                <select id="delivery_id" name="delivery_id" class="form-select mt-2 sm:mr-2"    >
                                    ...
                                    @foreach ($deliveries as $delivery)
                                        <option value="{{$delivery->id}}" {{$warehousetrans->delivery_id ==$delivery->id?'selected':''}}>{{$delivery->full_name}}</option>
...
<script src="{{asset('backend/assets/js/product_transfer.js')}}"></script> 
...
$(document).ready(function(){ //Your code here 
    $.ajax({
        type: 'GET',
        url: '{{route("warehousetrans.getProductList")}}',
        data: {
            wti_id: {{$warehousetrans->id}},
        ...
        success: function(data) {
            ...
            products.forEach((pitem) => {
                ...
                newpro = new Product(pitem.id,pitem.title, pitem.price,pitem.quantity,pitem.stock_qty +  pitem.quantity,imageurls[0]);
                flag = 0;
...
$('#btnstore').on( "click", function() {
    ...
    $.ajax({
        url: "{{route('warehousetransfer.update',$warehousetrans->id)}}", // Replace with your actual server endpoint URL
        method: "POST",
