                        ...
                        <div class="intro-y box mt-3  py-3 px-3 ">
                            <div class="mt-3">
                                ...
                                <select id="wh_id1" name="wh_id1" class="form-select mt-2 sm:mr-2"    >
                                    @foreach ($warehouses as $wh)
                                        <option value="{{$wh->id}}" {{old('wh_id1')==$wh->id?'selected':''}}>{{$wh->title}}</option>
                            ...
                            <div class="mt-3">
                                ...
                                <select id="vendor_id1" name="vendor_id1" class="form-select mt-2 sm:mr-2"    >
                                    @foreach ($vendors as $vd)
                                        <option value="{{$vd->id}}" {{old('vendor_id1')==$vd->id?'selected':''}}>{{$vd->full_name}}</option>
                            ...
                            <div class="mt-3">
                               ...
                               Kho đích
                                ...
                                <select id="wh_id2" name="wh_id2" class="form-select mt-2 sm:mr-2"    >
                                    @foreach ($warehouses as $wh)
                                        <option value="{{$wh->id}}" {{old('wh_id2')==$wh->id?'selected':''}}>{{$wh->title}}</option>
                            ...
                            <div class="mt-3">
                                ...
                                <select id="vendor_id2" name="vendor_id2" class="form-select mt-2 sm:mr-2"    >
                                    @foreach ($vendors as $vd)
                                        <option value="{{$vd->id}}" {{old('vendor_id2')==$vd->id?'selected':''}}>{{$vd->full_name}}</option>
                            ...
                            <div class="mt-3">
                                ...
                                <select id="delivery_id" name="delivery_id" class="form-select mt-2 sm:mr-2"    >
                                    ...
                                    @foreach ($deliveries as $delivery)
                                        <option value="{{$delivery->id}}" {{old('delivery_id')==$delivery->id?'selected':''}}>{{$delivery->full_name}}</option>
...
<script src="{{asset('backend/assets/js/product_transfer3.js')}}"></script> 
...
$('#btnstore').on( "click", function() {
    ...
    $.ajax({
        url: "{{route('warehousetransfer.store')}}", // Replace with your actual server endpoint URL
        method: "POST",
