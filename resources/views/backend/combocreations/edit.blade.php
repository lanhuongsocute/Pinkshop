                                ...
                                <div class="mt-3">
                                    ...
                                    <select id="warehouse_id" name="wh_id" class="form-select mt-2 sm:mr-2"    >
                                        @foreach ($warehouses as $wh)
                                            <option value="{{$wh->id}}" {{$combo->wh_id==$wh->id?'selected':''}}>{{$wh->title}}</option>
                                        @endforeach
                                </select>
                                </div>
                            ...
                            <div class="intro-y box mt-3  py-3 px-3 ">
                                ...
                                <div class="lg:flex intro-y" id="product_combo">
                                    <div style='padding-top:15px;padding-bottom:15px'> 
                                       <b> <span> Sản phẩm: {{$combo->title}}</span></b> 
                                    </div>
                                </div>
                             
                                <div class="relative">
                                        <label> Số lượng</label>
                                        <input type="number" id='quantity' class="form-control py-3 px-4 w-full lg:w-64 box pr-10" value="{{$combo->quantity}}">
                                         
                                </div>
                                <div class="relative">
                                    <label> Đơn giá bán</label>
                                    <input type="text" id='sold_price' class="form-control py-3 px-4 w-full lg:w-64 box pr-10"  oninput="formatNumberS(this)" value="{{number_format($combo->price,0,'.',',')}}">
                                    
                                </div>
                                <input type="hidden" id='product_id'  name="product_id" value = "{{$combo->product_id}}">
...
$(document).ready(function(){ //Your code here 
    ...
    $.ajax({
        type: 'GET',
        url: '{{route("combocreation.getProductList")}}',
        data: {
            combo_id: {{$combo->id}},
...
$('#btnstore').on( "click", function() {
    ...
    importDoc = new ImportDoc({{$combo->id}},final_amount, wh_id,product_id,quantity);
    ...
    $.ajax({
        url: "{{route('combocreation.update',$combo->id)}}", // Replace with your actual server endpoint URL
        method: "POST",
