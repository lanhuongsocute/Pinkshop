...
@include('backend.layouts.notification')
    ...
    <div class="grid grid-cols-12 gap-12 mt-5">
        <div class="intro-y col-span-12 lg:col-span-12">
            <!-- BEGIN: Form Layout -->
            <form method="post" action="{{route('maintaintodestroy.update',$mtd->id)}}">
                @csrf
                @method('patch')
                ...
                <div class="mt-3">
                         ...
                         <input id="product_search" value="{{\App\Models\Product::where('id',$mtd->product_id)->value('title')}}"  type="text" class="form-control" placeholder="tên">
                         <input type="hidden" id= "product_id" name="product_id" value = "{{$mtd->product_id}}"/>
                     </div>
                    ...
                    <div class="mt-3">
                        ...
                        <input  onchange="updateQuantity()"  class="form-control" type="text" id= "quantity" name="quantity" value='{{$mtd->quantity}}'/>
                        <div class="form-help">
                            (Tồn kho hiện tại: <span id="spstock">{{\App\Models\InventoryMaintenance::where('product_id',$mtd->product_id)->value('quantity') + $mtd->quantity}} </span> )
                        </div>
                    </div>
                    <div class="mt-3">
                        <label for="regular-form-1" class="form-label">Đơn giá</label>
                        <input   class="form-control" type="text" id= "price" name="price" value='{{$mtd->price}}'/>
                    ...
                    <div class="mt-3">
                        ...
                        <input id="seri" onchange="updateQuantityS()" class="form-control" type="text" id= "series" name="series"  
                        value="{{$series}}"/>
...
var product_id = {{$mtd->product_id}};
var quantity = {{$mtd->quantity}};
var stock = {{\App\Models\InventoryMaintenance::where('product_id',$mtd->product_id)->value('quantity') + $mtd->quantity}} ;
