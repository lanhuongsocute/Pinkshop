...
@include('backend.layouts.notification')
    <div class="intro-y flex items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">
            Điều chỉnh chuyển kho sử dụng
        </h2>
    </div>
    <div class="grid grid-cols-12 gap-12 mt-5">
        <div class="intro-y col-span-12 lg:col-span-12">
            <!-- BEGIN: Form Layout -->
            <form method="post" action="{{route('maintaintoproperty.update',$mtp->id)}}">
                @csrf
                @method('patch')
                ...
                <div class="mt-3">
                         ...
                         <input id="product_search" value="{{\App\Models\Product::where('id',$mtp->product_id)->value('title')}}"  type="text" class="form-control" placeholder="tên">
                         <input type="hidden" id= "product_id" name="product_id" value = "{{$mtp->product_id}}"/>
                     </div>
                    ...
                    <div class="mt-3">
                        ...
                        <input  onchange="updateQuantity()"  class="form-control" type="text" id= "quantity" name="quantity" value='{{$mtp->quantity}}'/>
                        <div class="form-help">
                            (Tồn kho hiện tại: <span id="spstock">{{\App\Models\InventoryMaintenance::where('product_id',$mtp->product_id)->value('quantity') + $mtp->quantity}} </span> )
                        </div>
                    </div>
                    <div class="mt-3">
                        <label for="regular-form-1" class="form-label">Đơn giá</label>
                        <input   class="form-control" type="text" id= "price" name="price" value='{{$mtp->price}}'/>
...
var product_id = {{$mtp->product_id}};
var quantity = {{$mtp->quantity}};
var stock = {{\App\Models\InventoryMaintenance::where('product_id',$mtp->product_id)->value('quantity') + $mtp->quantity}} ;
