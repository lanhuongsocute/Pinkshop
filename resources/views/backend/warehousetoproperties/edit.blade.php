...
@include('backend.layouts.notification')
    ...
    <div class="grid grid-cols-12 gap-12 mt-5">
        <div class="intro-y col-span-12 lg:col-span-12">
            <!-- BEGIN: Form Layout -->
            <form method="post" action="{{route('warehousetoproperty.update',$wtp->id)}}">
                @csrf
                @method('patch')
                ...
                <div class="mt-3">
                        <div id="div_parent_id" class="  flex flex-col sm:flex-row items-center">
                            ...
                            <select id="warehouse_id" name="wh_id" class="form-select mt-2 sm:mr-2" aria-label="Default select example"   >
                                @foreach ($warehouses as $wh)
                                    <option value="{{$wh->id}}" {{$wtp->wh_id==$wh->id?'selected':''}}>{{$wh->title}}</option>
                    ...
                    <div class="mt-3">
                        ...
                        <input id="product_search" value="{{\App\Models\Product::find($wtp->product_id)->title}}"  
                                type="text" class="form-control" placeholder="tên">
                        <input type="hidden" id= "product_id" value="{{$wtp->product_id}}" name="product_id"/>
                    </div>
                    
                    <div class="mt-3">
                        <label for="regular-form-1" class="form-label">Số lượng</label>
                        <input  onchange="updateQuantity()"   class="form-control" type="text" id= "quantity" name="quantity" value='{{$wtp->quantity}}'/>
                        <div class="form-help">
                            (Tồn kho hiện tại: <span id="spstock"> {{\App\Models\Inventory::where('product_id',$wtp->product_id)->where('wh_id',$wtp->wh_id)->first()->quantity +$wtp->quantity }}</span> )
                        </div>
                    </div>
                    <div class="mt-3">
                        <label for="regular-form-1" class="form-label">Đơn giá</label>
                        <input   class="form-control" type="text" id= "price" name="price" value='{{$wtp->price}}'/>
                    </div>
                    <div class="mt-3">
...
var product_id = {{$wtp->product_id}};
var quantity = {{$wtp->quantity}};
var stock = {{\App\Models\Inventory::where('product_id',$wtp->product_id)->where('wh_id',$wtp->wh_id)->first()->quantity +$wtp->quantity }};
