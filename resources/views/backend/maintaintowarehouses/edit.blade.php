...
@include('backend.layouts.notification')
    <div class="intro-y flex items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">
            Điều chỉnh chuyển kho bán hàng
        </h2>
    </div>
    <div class="grid grid-cols-12 gap-12 mt-5">
        <div class="intro-y col-span-12 lg:col-span-12">
            <!-- BEGIN: Form Layout -->
            <form method="post" action="{{route('maintaintowarehouse.update',$mtw->id)}}">
                @csrf
                @method('patch')
                ...
                <div class="mt-3">
                         ...
                         <input id="product_search" value="{{\App\Models\Product::where('id',$mtw->product_id)->value('title')}}"  type="text" class="form-control" placeholder="tên">
                         <input type="hidden" id= "product_id" name="product_id" value = "{{$mtw->product_id}}"/>
                     </div>
                <div class="mt-3">
                        <div id="div_parent_id" class="  flex flex-col sm:flex-row items-center">
                            <label style="min-width:50px  " class="form-select-label" for="status">Kho chuyển tới</label>
                            <select id="warehouse_id" name="wh_id" class="form-select mt-2 sm:mr-2" aria-label="Default select example"   >
                                @foreach ($warehouses as $wh)
                                    <option value="{{$wh->id}}" {{$mtw->wh_id==$wh->id?'selected':''}}>{{$wh->title}}</option>
                    ...
                    <div class="mt-3">
                        ...
                        <input  onchange="updateQuantity()"  class="form-control" type="text" id= "quantity" name="quantity" value='{{$mtw->quantity}}'/>
                        <div class="form-help">
                            (Tồn kho hiện tại: <span id="spstock">{{\App\Models\InventoryMaintenance::where('product_id',$mtw->product_id)->value('quantity') + $mtw->quantity}} </span> )
                        </div>
                    </div>
                    <div class="mt-3">
                        <label for="regular-form-1" class="form-label">Đơn giá</label>
                        <input   class="form-control" type="text" id= "price" name="price" value='{{$mtw->price}}'/>
                        <div class="form-help"> * nên để giá trị mặc định nếu không chắc về giá trị sản phẩm </div>
                    </div>
                    <div class="mt-3">
                        <label for="regular-form-1" class="form-label">Thời gian bảo hành còn</label>
                        <input   class="form-control" type="text" id= "time" name="time" value='{{$mtw->time}}'/>
...
var product_id = {{$mtw->product_id}};
var quantity = {{$mtw->quantity}};
var stock = {{\App\Models\InventoryMaintenance::where('product_id',$mtw->product_id)->value('quantity') + $mtw->quantity}} ;
