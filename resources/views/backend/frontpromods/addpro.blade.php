    ...
    <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
            <table class="table table-report -mt-2">
                ...
                <tbody>
                    ...
                    <tr class="intro-x">
                        <td>
                             {{ \App\Models\Product::find($item->pro_id)->title}}  
                        </td>
                        <td>
                             {{  $item->order_id }}  
                        </td>
                        ...
                        <td class="table-report__action w-56">
                            <div class="flex justify-center items-center">
                                ...
                                <a class="dropdown-item" href="{{route('modpro.up',[$item->id,$frontmod->id])}}" class="flex items-center mr-3" href="javascript:;">   Tăng vị trí</a> 
                                <br/> 
                                <a class="dropdown-item" href="{{route('modpro.down',[$item->id,$frontmod->id])}}" class="flex items-center mr-3" href="javascript:;">   Giảm vị trí</a> 
                                <br/>   
                                <a class="dropdown-item" href="{{route('cproduct.priceview',[$item->pro_id,$frontmod->id])}}" class="flex items-center mr-3" href="javascript:;">   Thiết lập giá</a> 
                                     
                                            <form action="{{route('modpro.removepro' )}}" method = "post">
                                                @csrf
...
<script src="{{asset('backend/assets/js/product_v4.js')}}"></script> 
<script>
...
$('#btnstore').on( "click", function() {
    ...
    $.ajax({
        url: "{{route('modpro.savepro')}}", // Replace with your actual server endpoint URL
        method: "POST",
...
var tong = 0;
    ...
    product_search.autocomplete({
        source: function(request, response) {
            ...
            $.ajax({
                type: 'GET',
                url: '{{route('product.productjmodsearch')}}',
                data: {
                    data: request.term,
