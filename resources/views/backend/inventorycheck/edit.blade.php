                            ...
                            <div id="div_parent_id" class=" flex    px-4   w-full lg:w-auto mt-3 lg:mt-0 ml-auto  ">
                                ...
                                <select id="wh_id" name="wh_id" class="form-control" aria-label="Default select example"   >
                                    @foreach ($warehouses as $wh)
                                        <option value="{{$wh->id}}" {{$ic->wh_id==$wh->id?'selected':''}}>{{$wh->title}}</option>
...
<script src="{{asset('backend/assets/js/product_incheck.js')}}"></script> 
...
$(document).ready(function(){ //Your code here 
    ...
    $.ajax({
        type: 'GET',
        url: '{{route("inventorycheck.getProductList")}}',
        data: {
            ic_id: {{$ic->id}},
        ...
        success: function(data) {
            ...
            products.forEach((pitem) => {
                ...
                pitem.quantity = pitem.stock_qty + pitem.operation*pitem.error;
                newpro = new Product(pitem.id,pitem.title, pitem.price,pitem.quantity,pitem.stock_qty  ,imageurls[0],pitem.series );
                flag = 0;
...
$('#btnstore').on( "click", function() {
    ...
    $.ajax({
        url: "{{route('inventorycheck.update',$ic->id)}}", // Replace with your actual server endpoint URL
        method: "POST",
