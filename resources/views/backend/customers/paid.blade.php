...
<div class="content">
    ...
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
                    <h2 class="text-lg font-medium mr-auto">
                        Thu tiền khách hàng: {{\App\Models\User::where('id',$customer->id)->value('full_name')}}
                    </h2>
    ...
    <div class="grid grid-cols-12 gap-12 mt-5">
        <div class="intro-y col-span-12 lg:col-span-12">
            <!-- BEGIN: Form Layout -->
            <form method="post" action="{{route('customer.storepaid')}}">
                @csrf
                        ...
                        <?php
                            ...
                            ?>
                             <div class="{{ $classname}}"> 
                                ...
                                {{number_format($customer->budget  ,0,'.',',')}}  ({{ $text_c}})<br/>
                        </div>
                    </div>
                    
                    <div>
                        <label for="regular-form-1" class="form-label">Số tiền </label>
                        <input id="paid_amount" name="paid_amount" value = "{{$customer->budget<0?-$customer->budget:$customer->budget}}"
